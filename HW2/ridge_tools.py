...
def fit_predict(data, features, method="plain", n_folds=10):
    ...
    for i in range(n_folds):
        ...
        weights, __ = cross_val_ridge(train_features, train_data, method=method, n_splits = n_folds)
...
def cross_val_ridge(
    ...
    for icv, (trn, val) in enumerate(kf.split(train_data)):
        ...
        r_cv += cost
        print('fold {} of {}, time taken: {}'.format(icv, n_splits,time.time()-tic))
    ...
    for idx_lambda in range(
        ...
        idx_vox = np.where(argmin_lambda == idx_lambda)[0]
        if len(idx_vox)>0:
